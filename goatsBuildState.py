...
# Set up Slack bot token and channel ID
SLACK_TOKEN = os.getenv("SLACK_TOKEN")
CHANNEL_ID = "C09RMGFL55F"
...
def get_recent_messages():
    try:
        ...
        # Filter out messages that contain "Build Failure Summary (Last 7 Days)"
        filtered_messages = [
...
def extract_failed_builds(messages):
    ...
    for msg in messages:
        text = msg.get('text', '')
        if "Failed" in text: # Only consider failed builds
...
def generate_summary(failed_builds):
    ...
    for build_name, build_info in failed_builds.items():
        ...
        if link:
            ...
            summary += f":warning: - *{build_name}*: {count} failure(s)\n"
    return summary

def post_summary_to_chat(summary):
    try:
...
# Remove failed builds that have succeeded
failed_builds = {
    name: info for name, info in failed_builds.items() 
    if build_statuses.get(name) != "Succeeded"
}

summary = generate_summary(failed_builds)
post_summary_to_chat(summary)
